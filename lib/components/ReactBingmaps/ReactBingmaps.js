...
var ReactBingmaps = function (_Component) {
	...
	_createClass(ReactBingmaps, [{
		...
		value: function reactBingmaps(props, Microsoft) {
				...
				irregularPolygons = props.irregularPolygons,
	...
	{
		key: 'createIrregularPolygons',
		value: function createIrregularPolygons(irregularPolygons, mapReference){
			if(map[mapReference] && irregularPolygons){
				...
				for(let irregularPolygonIndex = 0; irregularPolygonIndex < irregularPolygons.length; irregularPolygonIndex++){
					...
					if (option.withPushPin) {
						for(let pointIndex = 0; pointIndex < points.length; pointIndex++) {
							let point = points[pointIndex].map(String);
							let latLong = point.join(",");
							let location = Microsoft.Maps.Location.parseLatLong(latLong);
							map[mapReference].entities.push(new Microsoft.Maps.Pushpin(location, option));
						}
					}
